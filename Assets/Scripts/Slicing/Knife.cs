...
public class Knife : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        {
            //its gameover mate! play the final animation
            other.gameObject.SetActive(false);
            GetComponent<Animator>().enabled = true;
            // GameManager.gameManager.GameOver();
            this.enabled = false;
            GetComponent<KnifeControl>().animator.enabled = false;
            GetComponent<KnifeControl>().enabled = false;
            return;
        }
        ...
        {
            //make it yellow
            particles(other,Color.yellow);
        }
        else if (other.transform.CompareTag("melon"))
        {
